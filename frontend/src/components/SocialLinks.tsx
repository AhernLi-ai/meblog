    ...
    <div className="flex justify-center gap-4">
      {links.map((link, index) => (
        <div key={index} className="relative group">
          ...
          {/* Tooltip */}
          {link.url && (
            <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 px-2 py-1 bg-gray-900 dark:bg-gray-700 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
              {link.label}
            </div>
          )}
        </div>
