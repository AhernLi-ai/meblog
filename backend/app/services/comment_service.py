...
def build_comment_response(comment, include_email: bool = False) -> CommentResponse:
    ...
    replies = []
    if comment.replies:
        replies = [build_comment_response(r, include_email) for r in comment.replies]
...
def get_comments_service(
    ...
    comments, total = get_comments_by_post_slug(db, post_slug)
    include_email = is_admin(db, user)
    items = [build_comment_response(c, include_email) for c in comments]
...
def add_comment_service(
    ...
    comment = create_comment(db, comment_data, visitor_id)

    # Reload with replies
    comment.replies = []
    return build_comment_response(comment, include_email=False)
